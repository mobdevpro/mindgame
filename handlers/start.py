...
ONBOARDING_STEPS = [
    ...
    (
        ...
        "Зафиксируй его и получи TRGR. Ты уже не на автопилоте – "
        "ты наблюдаешь."
    ),
    (
        "⚡ <b>TRGR за осознанность</b>\n\n"
        "📝 Запись триггера – TRGR\n"
        "💭 Назвал эмоцию – ещё TRGR\n"
        "💡 Нашёл вывод – ещё больше TRGR\n"
...
async def ask_subscription(message: Message, state: FSMContext, bot: Bot):
    ...
    await message.answer(
        ...
        f"После подписки получишь <b>50 стартовых TRGR</b> 🎁",
        parse_mode="HTML",
...
async def check_subscription(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    await callback.message.edit_text(
        ...
        f"🎁 Начислено 50 стартовых TRGR\n"
        f"💰 Твой баланс: {balance} TRGR\n\n"
...
async def cmd_help(message: Message):
    await message.answer(
        ...
        "📊 <b>Прогресс</b> – TRGR, уровень, достижения\n"
        "✅ <b>Чек-ин</b> – быстрое состояние прямо сейчас\n\n"
        "Каждое осознанное действие приносит TRGR! 🎮",
        parse_mode="HTML",
        reply_markup=kb.main_menu()
    )
